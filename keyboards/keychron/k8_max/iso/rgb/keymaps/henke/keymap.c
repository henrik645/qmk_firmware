...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_iso_88(
         KC_ESC,                          KC_BRID,  KC_BRIU,  KC_MCTRL, KC_LNPAD, RGB_VAD,  RGB_VAI,  KC_MPRV,  KC_MPLY,  KC_MNXT,  KC_MUTE,  KC_VOLD,  KC_VOLU,  MAC_SLEEP_CHORD,  KC_SIRI,  RGB_MOD,
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
    case MAC_SLEEP_CHORD:
        if (record->event.pressed) {
            // Send Option + Command + Power (Eject)
        ...
        return false; // Don't let QMK handle it further
    }
